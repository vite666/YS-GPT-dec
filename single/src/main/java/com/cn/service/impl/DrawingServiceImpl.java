...
import com.cn.dto.DrawingPrompt.AddDrawingPromptDto;
import com.cn.dto.DrawingPrompt.DeleteDrawingPromptDto;
import com.cn.dto.GenerateDrawingDeleteDto;
import com.cn.dto.DrawingPrompt.UpdateDrawingPromptDto;
...
public class DrawingServiceImpl implements DrawingService {
    ...
    public TaskResultVo getDrawingTask(final String taskId) {
        ...
        if (redisUtils.doesItExist(assetTaskId)) {
            ...
            } else if (taskStructure.getDrawingType().equals(DrawingTypeEnum.MJ.getDec())) {

                return handleGetMjTask(taskStructure, taskId);
            } else
     ...
     */
    private TaskResultVo handleGetMjTask(final TaskStructure taskStructure, final String taskId) {
        // 获取当前进度
        final String curProgress = String.valueOf(redisUtils.hashGet(DrawingConstant.MJ_EXECUTION, taskId));
        if (!"null".equals(curProgress)) {
            HashMap<String, Object> map = new HashMap<>();
            map.put("progress", curProgress);
    ...
    private TaskResultVo handleTaskStatus(final TaskStructure taskStructure) {
        ...
        if (status.equals(DrawingStatusConstant.PENDING)) {
            ...
            // 画图扣费，MJ 0.35 / 张， DALL 0.15 / 每张
            if ("MJ".equals(taskStructure.getDrawingType())) {
                tsUser.setAccountBalance(tsUser.getAccountBalance().subtract(BigDecimal.valueOf(0.35)));
                balanceRecordMapper.insert(new BalanceRecords().setBalance(BigDecimal.valueOf(0.35)).setUserId(userID).setType("消费").setPromptTokens(0l).setCompletionTokens(0l).setModelName("Midjourney"));
            }else if ("DALL".equals(taskStructure.getDrawingType())) {
                tsUser.setAccountBalance(tsUser.getAccountBalance().subtract(BigDecimal.valueOf(0.15)));
                balanceRecordMapper.insert(new BalanceRecords().setBalance(BigDecimal.valueOf(0.15)).setUserId(userID).setType("消费").setPromptTokens(0l).setCompletionTokens(0l).setModelName("DALL-3"));
            }
