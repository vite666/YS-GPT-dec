...
import {reqAddProductRecharge, reqDeleteProductRecharge, reqGetProductRechargePage, reqUpdateProductRecharge} from "@/api/productManagement";
...
const form = ref({
  ...
  productRechargeId:undefined,
  productName: '',
  days: undefined,
  price: undefined,
  rechargeQuota: undefined
})

const openUpdateDialog = (data) => {
  // form.value.productCardId = data.productCardId
  form.value.productRechargeId = data.productRechargeId;
  form.value.productName = data.productName
  // form.value.days = data.days
  form.value.price = data.price
  form.value.rechargeQuota = data.rechargeQuota;
...
const handleUpdateProduct = async () => {
  ...
  //   return
  // }
  if (!form.value.rechargeQuota) {
    ElNotification({message: '商品所含充值余额不能为空', type: "error", });
    return
  }
  let service = ElLoading.service({fullscreen: true, text: '请稍等'})
  try {
    await reqUpdateProductRecharge(form.value);
    //刷新数据
    await init(1, '')
    ElNotification({
      message: '修改成功',
...
const handleAddProductCard = async () => {
  ...
  let service = ElLoading.service({fullscreen: true, text: '请稍等'})
  try {
    await reqAddProductRecharge(form.value);
    //刷新数据
    await init(1, '')
    ElNotification({
      message: '添加成功',
...
const handleDeleteProduct = async (id) => {
  ...
  try {
    await reqDeleteProductRecharge({productRechargeId: id});
    //刷新数据
    await init(1, '')
    ElNotification({
      message: '删除成功',
...
const init = async (pageNum, prompt) => {
  ...
  try {
    const {data} = await reqGetProductRechargePage(pageNum, prompt);
    if (data.records) {
...
<template>
  <div class="container">
    <div class="body">
      ...
      >
        <el-table-column prop="productRechargeId" label="序号"/>
        <el-table-column prop="productName" label="商品名称" width="200"/>
        <el-table-column prop="price" label="价格" width="200"/>
        <el-table-column prop="rechargeQuota" label="充值额度" width="150">
          <template #default="scope">
            <div class="column-flex">
              <div class="type-default">
                {{ scope.row.rechargeQuota }} 元
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="200px">
          <template #default="scope">
            ...
            <el-button link type="primary" size="small" style="color: #ff3b3b"
                       @click="handleDeleteProduct(scope.row.productRechargeId)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
<!--        </div>-->
        <el-form-item label="所含额度">
          <el-input placeholder="请设置所含额度"
                    v-model="form.rechargeQuota"
                    type="number"
                    class="el-width"/>
        </el-form-item>
        <div class="prompt">
          例如 想上架一张5元额度充值 则设置 数字 <span>5</span> 即可
        </div>
